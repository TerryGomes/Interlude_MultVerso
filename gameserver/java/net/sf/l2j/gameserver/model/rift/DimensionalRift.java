...
public class DimensionalRift
{
	...
	public void manualTeleport(Player player, Npc npc)
	{
		...
		{
			DimensionalRiftManager.getInstance().showHtmlFile(player, player.isLang() + "seven_signs/rift/NotPartyLeader.htm", npc);
			return;
		}

		if (_currentJumps == Config.RIFT_MAX_JUMPS)
		{
			DimensionalRiftManager.getInstance().showHtmlFile(player, player.isLang() + "seven_signs/rift/UsedAllJumps.htm", npc);
			return;
		}

		if (_hasJumped)
		{
			DimensionalRiftManager.getInstance().showHtmlFile(player, player.isLang() + "seven_signs/rift/AlreadyTeleported.htm", npc);
			return;
		}
		_hasJumped = true;
