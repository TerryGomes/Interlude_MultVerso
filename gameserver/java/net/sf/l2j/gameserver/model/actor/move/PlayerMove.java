...
public class PlayerMove extends CreatureMove<Player>
{
	...
	public boolean updatePosition(boolean firstRun)
	{
		...
		if (_task == null || !_actor.isVisible() || (_pawn != null && !_actor.knows(_pawn)))
		{
			return true;
		}
		...
		if (type == MoveType.GROUND && !GeoEngine.getInstance().canMoveToTarget(curX, curY, curZ, nextX, nextY, nextZ) && !_actor.temporaryFixPagan())
		{
			_blocked = true;
			return true;
		}
	...
	{
		if (offset < 0 || _actor == target || _actor.isIn3DRadius(target, (int) (offset + _actor.getCollisionRadius() + ((target instanceof Creature) ? ((Creature) target).getCollisionRadius() : 0))))
		{
			return false;
		}
	...
	protected Location calculatePath(int ox, int oy, int oz, int tx, int ty, int tz)
	{
		...
		if ((moveType != MoveType.GROUND) || _actor.temporaryFixPagan())
		{
