...
public class ManorManagerNpc extends Merchant
{
	...
	{
		return player.isLang() + "manormanager/manager.htm";
	}
	...
	public void showChatWindow(Player player)
	{
		...
		{
			showChatWindow(player, player.isLang() + "npcdefault.htm");
			return;
		}

		if (getCastle() != null && player.getClan() != null && getCastle().getOwnerId() == player.getClanId() && player.isClanLeader())
		{
			showChatWindow(player, player.isLang() + "manormanager/manager-lord.htm");
		}
		else
		{
			showChatWindow(player, player.isLang() + "manormanager/manager.htm");
		}
	}
}
