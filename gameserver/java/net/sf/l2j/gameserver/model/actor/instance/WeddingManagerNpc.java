...
public class WeddingManagerNpc extends Folk
{
	...
	public void onInteract(Player player)
	{
		...
		{
			sendHtmlMessage(player, player.isLang() + "mods/wedding/start2.htm");
		}
		else if (player.isUnderMarryRequest())
		{
			sendHtmlMessage(player, player.isLang() + "mods/wedding/waitforpartner.htm");
			// And normal players go here :)
		}
		else
		{
			sendHtmlMessage(player, player.isLang() + "mods/wedding/start.htm");
		}
	}
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (command.startsWith("AskWedding"))
		{
			...
			if (st.hasMoreTokens())
			{
				...
				if (partner == null)
				{
					sendHtmlMessage(player, player.isLang() + "mods/wedding/notfound.htm");
					return;
				}
	...
	private boolean weddingConditions(Player requester, Player partner)
	{
		...
		{
			sendHtmlMessage(requester, requester.isLang() + "mods/wedding/error_wrongtarget.htm");
			return false;
		}
		...
		{
			sendHtmlMessage(requester, requester.isLang() + "mods/wedding/error_sex.htm");
			return false;
		}
		...
		{
			sendHtmlMessage(requester, requester.isLang() + "mods/wedding/error_friendlist.htm");
			return false;
		}
		...
		{
			sendHtmlMessage(requester, requester.isLang() + "mods/wedding/error_alreadymarried.htm");
			return false;
		}
		...
		{
			sendHtmlMessage(requester, requester.isLang() + "mods/wedding/error_noformal.htm");
			return false;
		}
		...
		{
			sendHtmlMessage(requester, requester.isLang() + "mods/wedding/error_adena.htm");
			return false;
		}
