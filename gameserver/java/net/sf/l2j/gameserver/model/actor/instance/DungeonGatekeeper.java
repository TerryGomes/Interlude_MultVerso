...
public class DungeonGatekeeper extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if (command.startsWith("necro"))
		{
			...
			if (!canPort)
			{
				...
				html.setFile(player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + "necro_no.htm");
		...
		else if (command.startsWith("cata"))
		{
			...
			{
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile(player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + "cata_no.htm");
				player.sendPacket(html);
			}
			else
			{
	...
	public String getHtmlPath(Player player, int npcId, int val)
	{
		...
		return player.isLang() + "gatekeeper/" + filename + ".htm";
	}
}