		...
		switch (state)
		{
			case NONE: // Remove Punishments
				switch (_type)
				{
					...
					case JAIL:
						...
						if (!CTFEvent.isInactive() && CTFEvent.isPlayerParticipant(_owner.getObjectId()))
						{
							CTFEvent.removeParticipant(_owner.getObjectId());
						}

						if (!DMEvent.isInactive() && DMEvent.isPlayerParticipant(_owner.getObjectId()))
						{
							DMEvent.removeParticipant(_owner);
						}

						if (!LMEvent.isInactive() && LMEvent.isPlayerParticipant(_owner.getObjectId()))
						{
							LMEvent.removeParticipant(_owner);
						}

						if (!TvTEvent.isInactive() && TvTEvent.isPlayerParticipant(_owner.getObjectId()))
						{
							TvTEvent.removeParticipant(_owner.getObjectId());
						}

						// Open a Html message to inform the player
						final NpcHtmlMessage html = new NpcHtmlMessage(0);
						html.setFile(_owner.isLang() + "jail_out.htm");
			...
			case JAIL: // Jail Player
				...
				if (delayInMinutes > 0)
				{
					...
					_owner.sendMessage(new CustomMessage("PUNISHMENT_TIME", delayInMinutes));
				}
				...
				// Open a Html message to inform the player
				final NpcHtmlMessage html = new NpcHtmlMessage(0);
				html.setFile(_owner.isLang() + "jail_in.htm");
				_owner.sendPacket(html);
