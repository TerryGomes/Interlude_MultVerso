...
public class Adventurer extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (command.startsWith("raidInfo"))
		{
			...
			String filename = player.isLang() + "adventurer_guildsman/raid_info/info.htm";
			if (bossLevel != 0)
			{
				filename = player.isLang() + "adventurer_guildsman/raid_info/level" + bossLevel + ".htm";
			}
	...
	public String getHtmlPath(Player player, int npcId, int val)
	{
		...
		return player.isLang() + "adventurer_guildsman/" + filename + ".htm";
	}
}