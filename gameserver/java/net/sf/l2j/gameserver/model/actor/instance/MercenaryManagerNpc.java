...
public final class MercenaryManagerNpc extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.startsWith("how_to"))
		{
			...
			html.setFile(player.isLang() + "mercmanager/mseller005.htm");
		...
		else if (command.startsWith("hire"))
		{
			...
			{
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile(player.isLang() + "mercmanager/msellerdenial.htm");
				html.replace("%objectId%", getObjectId());
				player.sendPacket(html);
				return;
			}
			...
			final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			html.setFile(player.isLang() + "mercmanager/mseller004.htm");
			player.sendPacket(html);
		}
		else if (command.startsWith("merc_limit"))
		{
			final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			html.setFile(player.isLang() + "mercmanager/" + ((getCastle().getCastleId() == 5) ? "aden_msellerLimit.htm" : "msellerLimit.htm"));
	...
	public void showChatWindow(Player player)
	{
		...
		{
			html.setFile(player.isLang() + "mercmanager/mseller002.htm");
		}
		else if (condition == NpcTalkCond.UNDER_SIEGE)
		{
			html.setFile(player.isLang() + "mercmanager/mseller003.htm");
		}
		else
		{
			...
			{
				case DAWN:
					html.setFile(player.isLang() + "mercmanager/mseller001_dawn.htm");
					break;

				case DUSK:
					html.setFile(player.isLang() + "mercmanager/mseller001_dusk.htm");
					break;

				default:
					html.setFile(player.isLang() + "mercmanager/mseller001.htm");
					break;
			}
		}
