...
public class Cubic
{
	...
	private Creature pickEnemyTarget()
	{
		...
		if (_owner.isInDuel())
		{
			...
			else if (playerA != _owner && ownerTarget == playerA)
			{
				target = playerA;
			}
		...
		else if (ownerTarget instanceof Creature && ownerTarget != _owner.getSummon() && ownerTarget != _owner)
		{
			...
			{
				if ((((Attackable) ownerTarget).getAggroList().get(_owner) != null) || (_owner.getSummon() != null && ((Attackable) ownerTarget).getAggroList().get(_owner.getSummon()) != null))
				{
					target = (Creature) ownerTarget;
				}
			}
			else if (ownerTarget instanceof Playable && ((_owner.getPvpFlag() > 0 && !_owner.isInsideZone(ZoneId.PEACE)) || _owner.isInsideZone(ZoneId.PVP)))
			{
				...
				if (enemy != null)
				{
					...
					if ((enemy.getPvpFlag() == 0 && !enemy.isInsideZone(ZoneId.PVP)) || enemy.isInsideZone(ZoneId.PEACE))
					{
						return null;
					}
	...
	private void useDisablerSkill(L2Skill skill, Creature target)
	{
		...
		else // If this is a debuff, let the duel manager know about it so the debuff can be removed after the duel (player & target must be in the same duel)
