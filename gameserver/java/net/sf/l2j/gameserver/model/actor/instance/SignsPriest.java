...
public class SignsPriest extends Folk
{
		...
		else if (command.startsWith("SevenSigns"))
		{
			...
			switch (value)
			{
				...
				case 33: // "I wish to participate.", "Back", [SevenSigns 33 cabal]
					...
					if (!Config.SEVEN_SIGNS_BYPASS_PREREQUISITES)
					{
						...
						if (classLevel > 1)
						{
							...
							{
								showChatWindow(player, player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + "signs_33_dusk_no.htm");
								return;
							}
							...
							{
								showChatWindow(player, player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + "signs_33_dawn_fee.htm");
								return;
							}
						}
					}
				...
				case 34: // "Pay the participation fee.", [SevenSigns 34 cabal] (cabal is not used, it is only for Dawn Priest)
					...
					final boolean canPayFee = ((player.getClassId().getLevel() > 1) && adena != null && adena.getCount() >= SevenSignsManager.ADENA_JOIN_DAWN_COST) || (certif != null && certif.getCount() >= 1);

					showChatWindow(player, player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + ((canPayFee) ? "signs_33_dawn.htm" : "signs_33_dawn_no.htm"));
					break;
				...
				case 4: // "I wish to fight for the Seal of Avarice/Gnosis/Strife.", [SevenSigns 4 cabal seal]
					...
					if (!Config.SEVEN_SIGNS_BYPASS_PREREQUISITES)
					{
						...
						if (cabal == CabalType.DAWN && (player.getClassId().getLevel() > 1 && !hasCastle && !player.destroyItemByItemId("SevenSigns", SevenSignsManager.CERTIFICATE_OF_APPROVAL_ID, 1, this, false) && !player.reduceAdena("SevenSigns", SevenSignsManager.ADENA_JOIN_DAWN_COST, this, false)))
						{
							showChatWindow(player, player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + "signs_33_dawn_no.htm");
							return;
						}
					}
				...
				case 6: // "Contribute Blue/Green/Red/all of my Seal Stones.", "Receive bonuses for gathering seal stones." [SevenSigns 6 stone]
					...
					else
					{
						...
						html.setFile(player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + ((this instanceof DawnPriest) ? "signs_6_dawn_contribute.htm" : "signs_6_dusk_contribute.htm"));
				...
				case 7: // Exchange Ancient Adena for Adena at Black Marketeer of Mammon, [SevenSigns 7 adena]
					...
					{
						showChatWindow(player, player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + "blkmrkt_3.htm");
						return;
					}
					...
					{
						showChatWindow(player, player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + "blkmrkt_4.htm");
						return;
					}
					...
					showChatWindow(player, player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + "blkmrkt_5.htm");
					break;
				...
				case 17: // "Exchange Blue/Green/Red/all Seal Stones.", [SevenSigns 17 stone]
					...
					html.setFile(player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH + ((this instanceof DawnPriest) ? "signs_17_dawn.htm" : "signs_17_dusk.htm"));
					html.replace("%stoneColor%", stoneColor);
	...
	public void showChatWindow(Player player, int val)
	{
		...
		String filename = player.isLang() + SevenSignsManager.SEVEN_SIGNS_HTML_PATH;
