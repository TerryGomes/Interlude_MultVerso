...
public abstract class Playable extends Creature
{
	...
	public boolean doDie(Creature killer)
	{
		...
		else if (isNoblesseBlessed() || (LMEvent.isStarted() && Config.LM_EVENT_EFFECTS_REMOVAL == 2 || TvTEvent.isStarted() && Config.TVT_EVENT_EFFECTS_REMOVAL == 2 || DMEvent.isStarted() && Config.DM_EVENT_EFFECTS_REMOVAL == 2 || CTFEvent.isStarted() && Config.CTF_EVENT_EFFECTS_REMOVAL == 2))
		{
			stopNoblesseBlessing(null);
	...
	public boolean canCastOffensiveSkillOnPlayable(Playable target, L2Skill skill, boolean isCtrlPressed)
	{
		...
		if (isInSameActiveOlympiadMatch(targetPlayer) || isInSameActiveDuel(targetPlayer))
		{
			return true;
		}
		...
		if (CTFEvent.isStarted() || TvTEvent.isStarted() || LMEvent.isStarted() || DMEvent.isStarted() && getActingPlayer().getTeam() == targetPlayer.getTeam())
		{
			return true;
		}
	...
	public boolean isAttackableBy(Creature attacker)
	{
		...
		if (attacker instanceof Playable)
		{
			...
			if ((getActingPlayer().isCursedWeaponEquipped() && attackerPlayable.getStatus().getLevel() <= 20) || (attackerPlayable.getActingPlayer().isCursedWeaponEquipped() && getStatus().getLevel() <= 20))
			{
				return false;
			}
		}
	...
	public boolean isAttackableWithoutForceBy(Playable attacker)
	{
		...
		// No checks for players in Olympiad.
		// No checks for players in Duel.
		if (isInSameActiveOlympiadMatch(attackerPlayer) || isInSameActiveDuel(attackerPlayer))
		{
			return true;
		}
		...
		if (CTFEvent.isStarted() || TvTEvent.isStarted() || LMEvent.isStarted() || DMEvent.isStarted() && getActingPlayer().getTeam() == attacker.getActingPlayer().getTeam())
		{
			return true;
		}
	...
	public boolean canKeepAttacking(Creature target)
	{
		...
		if (target instanceof Playable)
		{
			...
			if ((targetPlayer.getKarma() > 0) || isInSameActiveOlympiadMatch(targetPlayer) || isInSameActiveDuel(targetPlayer))
			{
				return true;
			}
	...
	{
		return getActingPlayer().getClanId() > 0 && getActingPlayer().getClanId() == playable.getActingPlayer().getClanId();
	}
	...
	public final boolean isInSameActiveSiegeSide(Playable playable)
	{
		...
		final Siege siege = CastleManager.getInstance().getActiveSiege(this);
		if ((siege == null) || getActingPlayer().getClan() != null || playable.getActingPlayer().getClan() != null)
		{
			return false;
		}

		// Return true if both this and tested Playable share same side, false otherwise.
		return !siege.isOnOppositeSide(getActingPlayer().getClan(), playable.getActingPlayer().getClan());
	}
	...
	public final boolean isAtWarWith(Playable playable)
	{
		...
		final Clan tClan = playable.getActingPlayer().getClan();
