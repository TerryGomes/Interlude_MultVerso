...
public class CastleMagician extends Folk
{
	...
	public void showChatWindow(Player player, int val)
	{
		...
		{
			html.setFile(player.isLang() + "castlemagician/magician-no.htm");
		}
		else if (condition == NpcTalkCond.UNDER_SIEGE)
		{
			html.setFile(player.isLang() + "castlemagician/magician-busy.htm");
		}
		else if (val == 0)
		{
			html.setFile(player.isLang() + "castlemagician/magician.htm");
		}
		else
		{
			html.setFile(player.isLang() + "castlemagician/magician-" + val + ".htm");
		}
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.equals("gotoleader"))
		{
			if (player.getClan() != null)
			{
				...
				String filename = player.isLang() + "castlemagician/magician-nogate.htm";
