...
public class Merchant extends Folk
{
	...
	public String getHtmlPath(Player player, int npcId, int val)
	{
		...
		return player.isLang() + "merchant/" + filename + ".htm";
	}
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (actualCommand.equalsIgnoreCase("Sell"))
		{
			...
			if (items.isEmpty())
			{
				final String content = HtmCache.getInstance().getHtm(player.isLang() + "" + ((this instanceof Fisherman) ? "fisherman" : "merchant") + "/" + getNpcId() + "-empty.htm");
				if (content != null)
				{
		...
		else if (actualCommand.equalsIgnoreCase("Multisell_Shadow"))
		{
			...
			{
				html.setFile(player.isLang() + "common/shadow_item-lowlevel.htm");
			}
			else if (player.getStatus().getLevel() < 46)
			{
				html.setFile(player.isLang() + "common/shadow_item_mi_c.htm");
			}
			else if (player.getStatus().getLevel() < 52)
			{
				html.setFile(player.isLang() + "common/shadow_item_hi_c.htm");
			}
			else
			{
				html.setFile(player.isLang() + "common/shadow_item_b.htm");
			}
		...
		else if (actualCommand.equalsIgnoreCase("Newbie_Exc_Multisell"))
		{
			...
			{
				showChatWindow(player, player.isLang() + "exchangelvlimit.htm");
			}
		}
		else
		{
