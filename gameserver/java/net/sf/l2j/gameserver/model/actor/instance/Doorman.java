...
public class Doorman extends Folk
{
	...
	public void showChatWindow(Player player)
	{
		...
		html.setFile(player.isLang() + "doormen/" + getTemplate().getNpcId() + ((!isOwnerClan(player)) ? "-no.htm" : ".htm"));
	...
	{
		String path = player.isLang() + "doormen/" + getNpcId() + "-busy.htm";
		if (!HtmCache.getInstance().isLoadable(path))
		{
			path = player.isLang() + "doormen/busy.htm";
		}
