...
public class OlympiadManagerNpc extends Folk
{
	...
	public void showChatWindow(Player player, int val)
	{
		...
		html.setFile(player.isLang() + "olympiad/" + filename);
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (command.startsWith("OlympiadNoble"))
		{
			...
			{
				html.setFile(player.isLang() + Olympiad.OLYMPIAD_HTML_PATH + "noble_cant_cw.htm");
				player.sendPacket(html);
				return;
			}
			...
			{
				html.setFile(player.isLang() + Olympiad.OLYMPIAD_HTML_PATH + "noble_cant_sub.htm");
				html.replace("%objectId%", getObjectId());
				player.sendPacket(html);
				return;
			}
			...
			{
				html.setFile(player.isLang() + Olympiad.OLYMPIAD_HTML_PATH + "noble_cant_thirdclass.htm");
				html.replace("%objectId%", getObjectId());
				player.sendPacket(html);
				return;
			}
			...
			switch (val)
			{
				...
				case 2: // Show waiting list
					...
					html.setFile(player.isLang() + Olympiad.OLYMPIAD_HTML_PATH + "noble_registered.htm");
				...
				case 3: // There are %points% Grand Olympiad points granted for this event.
					...
					html.setFile(player.isLang() + Olympiad.OLYMPIAD_HTML_PATH + "noble_points1.htm");
				...
				case 6: // request tokens reward
					html.setFile(player.isLang() + Olympiad.OLYMPIAD_HTML_PATH + ((Olympiad.getInstance().getNoblessePasses(player, false) > 0) ? "noble_settle.htm" : "noble_nopoints2.htm"));
					html.replace("%objectId%", getObjectId());
					player.sendPacket(html);
					break;
			...
			switch (val)
			{
				case 2: // Show rank for a specific class, example >> Olympiad 1_88
					...
					if (classId >= 88 && classId <= 118)
					{
						...
						html.setFile(player.isLang() + Olympiad.OLYMPIAD_HTML_PATH + "noble_ranking.htm");
				...
				case 3: // Spectator overview
					html.setFile(player.isLang() + Olympiad.OLYMPIAD_HTML_PATH + "olympiad_observe_list.htm");
				...
				case 5: // Hero pending state.
					if (HeroManager.getInstance().isInactiveHero(player.getObjectId()))
					{
						html.setFile(player.isLang() + Olympiad.OLYMPIAD_HTML_PATH + "hero_confirm.htm");
						html.replace("%objectId%", getObjectId());
						player.sendPacket(html);
					}
					break;
				...
				case 7: // Main panel
					html.setFile(player.isLang() + Olympiad.OLYMPIAD_HTML_PATH + "hero_main.htm");
