...
public class CastleWarehouseKeeper extends WarehouseKeeper
{
	...
	public void showChatWindow(Player player, int val)
	{
		...
		{
			html.setFile(player.isLang() + "castlewarehouse/castlewarehouse-no.htm");
		}
		else if (condition == NpcTalkCond.UNDER_SIEGE)
		{
			html.setFile(player.isLang() + "castlewarehouse/castlewarehouse-busy.htm");
		}
		else if (val == 0)
		{
			html.setFile(player.isLang() + "castlewarehouse/castlewarehouse.htm");
		}
		else
		{
			html.setFile(player.isLang() + "castlewarehouse/castlewarehouse-" + val + ".htm");
		}
