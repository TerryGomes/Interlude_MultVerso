...
public class Duel
{
	...
	protected DuelResult checkEndDuelCondition()
	{
		...
		if (!_isPartyDuel)
		{
			...
			if (_playerA.getDuelState() == DuelState.INTERRUPTED || _playerB.getDuelState() == DuelState.INTERRUPTED || !_playerA.isIn3DRadius(_playerB, 2000))
			{
				return DuelResult.CANCELED;
			}
		...
		else
		{
			if (_playerA.getParty() != null)
			{
				for (Player partyMember : _playerA.getParty().getMembers())
				{
					...
					// Players are too far apart.
					// One of the players is engaged in PvP.
					if ((partyMember.getDuelState() == DuelState.INTERRUPTED) || !partyMember.isIn3DRadius(_playerB, 2000) || (partyMember.getPvpFlag() != 0))
					{
						return DuelResult.CANCELED;
					}
			...
			if (_playerB.getParty() != null)
			{
				for (Player partyMember : _playerB.getParty().getMembers())
				{
					...
					// Players are too far apart.
					// One of the players is engaged in PvP.
					if ((partyMember.getDuelState() == DuelState.INTERRUPTED) || !partyMember.isIn3DRadius(_playerA, 2000) || (partyMember.getPvpFlag() != 0))
					{
						return DuelResult.CANCELED;
					}
