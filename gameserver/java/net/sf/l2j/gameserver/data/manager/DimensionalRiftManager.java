...
public class DimensionalRiftManager implements IXmlReader
{
	...
	public void load()
	{
		...
		LOGGER.info("Loaded {} Dimensional Rift rooms.", _rooms.size());
	}
	...
	public synchronized void start(Player player, byte type, Npc npc)
	{
		...
		if (party == null)
		{
			showHtmlFile(player, player.isLang() + "seven_signs/rift/NoParty.htm", npc);
			return;
		}
		...
		{
			showHtmlFile(player, player.isLang() + "seven_signs/rift/NotPartyLeader.htm", npc);
			return;
		}
		...
		if (party.getMembersCount() < Config.RIFT_MIN_PARTY_SIZE)
		{
			...
			html.setFile(player.isLang() + "seven_signs/rift/SmallParty.htm");
		...
		{
			final NpcHtmlMessage html = new NpcHtmlMessage(npc.getObjectId());
			html.setFile(player.isLang() + "seven_signs/rift/Full.htm");
			html.replace("%npc_name%", npc.getName());
			player.sendPacket(html);
			return;
		}
		...
		for (Player member : party.getMembers())
		{
			...
			{
				showHtmlFile(player, player.isLang() + "seven_signs/rift/NotInWaitingRoom.htm", npc);
				return;
			}
		}
		...
		for (Player member : party.getMembers())
		{
			...
			{
				final NpcHtmlMessage html = new NpcHtmlMessage(npc.getObjectId());
				html.setFile(player.isLang() + "seven_signs/rift/NoFragments.htm");
				html.replace("%npc_name%", npc.getName());
