...
public class SevenSignsManager
{
	...
	protected void setCalendarForNextPeriodChange()
	{
		switch (_activePeriod)
		{
			case SEAL_VALIDATION, COMPETITION ->
			{
				...
				if (daysToChange == 7)
				{
					if ((_nextPeriodChange.get(Calendar.HOUR_OF_DAY) < PERIOD_START_HOUR) || (_nextPeriodChange.get(Calendar.HOUR_OF_DAY) == PERIOD_START_HOUR && _nextPeriodChange.get(Calendar.MINUTE) < PERIOD_START_MINS))
					{
						daysToChange = 0;
					}
				}
	...
	public final int getCurrentScore(CabalType cabal)
	{
		...
		return switch (cabal)
		{
			case DAWN -> Math.round((float) (_dawnStoneScore / ((float) totalStoneScore == 0 ? 1 : totalStoneScore)) * 500) + getCurrentFestivalScore(cabal);
			case DUSK -> Math.round((float) (_duskStoneScore / ((float) totalStoneScore == 0 ? 1 : totalStoneScore)) * 500) + getCurrentFestivalScore(cabal);
			default -> 0;
		};
	}
