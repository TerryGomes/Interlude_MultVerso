...
public class Q420_LittleWing extends Quest
{
	...
	private static boolean checkItems(Player player, boolean isDeluxe)
	{
		...
		else if (player.getInventory().getItemCount(GEMSTONE_D) >= 1 && player.getInventory().getItemCount(SILVER_NUGGET) >= 3 && player.getInventory().getItemCount(TOAD_LORD_BACK_SKIN) >= 10)
		{
			return true;
		}
		return false;
