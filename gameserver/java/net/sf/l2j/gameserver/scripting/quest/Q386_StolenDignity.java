...
public class Q386_StolenDignity extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.startsWith("select_1-")) // first pick
		{
			...
			htmltext = fillBoard(st, getHtmlText(player, "30843-13.htm"));
		}
		else if (event.startsWith("select_2-")) // pick #2-5
		{
			...
			{
				htmltext = fillBoard(st, getHtmlText(player, "30843-" + (13 + 2 * playerArray.length()) + ".htm"));
			}
			else
			{
				...
				htmltext = fillBoard(st, getHtmlText(player, "30843-" + (12 + 2 * playerArray.length()) + ".htm"));
			}
		}
		else if (event.startsWith("select_3-")) // pick #6
		{
			...
			if (playerArray.contains(number))
			{
				htmltext = fillBoard(st, getHtmlText(player, "30843-25.htm"));
			}
			else
			{
				...
				if (winningLines == 3)
				{
					htmltext = getHtmlText(player, "30843-22.htm");
					giveItems(player, Rnd.get(REWARDS), 4);
				}
				else if (winningLines == 0)
				{
					htmltext = getHtmlText(player, "30843-24.htm");
					giveItems(player, Rnd.get(REWARDS), 10);
				}
				else
				{
					htmltext = getHtmlText(player, "30843-23.htm");
				}
