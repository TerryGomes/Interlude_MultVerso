...
public class Q384_WarehouseKeepersPastime extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.startsWith("select_1-")) // first pick
		{
			...
			htmltext = fillBoard(st, getHtmlText(player, npcId + "-14.htm"));
		}
		else if (event.startsWith("select_2-")) // pick #2-5
		{
			...
			{
				htmltext = fillBoard(st, getHtmlText(player, npcId + "-" + (14 + 2 * playerArray.length()) + ".htm"));
			}
			else
			{
				...
				htmltext = fillBoard(st, getHtmlText(player, npcId + "-" + (13 + 2 * playerArray.length()) + ".htm"));
			}
		}
		else if (event.startsWith("select_3-")) // pick #6
		{
			...
			if (playerArray.contains(number))
			{
				htmltext = fillBoard(st, getHtmlText(player, npcId + "-26.htm"));
			}
			else
			{
				...
				if (winningLines == 3)
				{
					htmltext = getHtmlText(player, npcId + "-23.htm");
				...
				{
					htmltext = getHtmlText(player, npcId + "-25.htm");
				...
				{
					htmltext = getHtmlText(player, npcId + "-24.htm");
				}
