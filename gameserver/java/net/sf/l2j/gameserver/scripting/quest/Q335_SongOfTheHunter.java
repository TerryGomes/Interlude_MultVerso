	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("Tor_list_1"))
		{
			...
			if (st.getInteger("tor") == 0)
			{
				...
				htmltext = getHtmlText(player, "30745-16.htm").replace("%reply0%", TOR_REQUEST_1_MENU[reply0]).replace("%reply1%", TOR_REQUEST_1_MENU[reply1]).replace("%reply2%", TOR_REQUEST_1_MENU[reply2]).replace("%reply3%", TOR_REQUEST_1_MENU[reply3]).replace("%reply4%", TOR_REQUEST_1_MENU[reply4]);
			}
		}
		else if (event.equalsIgnoreCase("Tor_list_2"))
		{
			if (st.getInteger("tor") == 0)
			{
				...
				htmltext = getHtmlText(player, "30745-16.htm").replace("%reply0%", TOR_REQUEST_2_MENU[reply0]).replace("%reply1%", TOR_REQUEST_2_MENU[reply1]).replace("%reply2%", TOR_REQUEST_2_MENU[reply2]).replace("%reply3%", TOR_REQUEST_2_MENU[reply3]).replace("%reply4%", TOR_REQUEST_2_MENU[reply4]);
			}
		}
				...
				switch (npc.getNpcId())
				{
					case GREY:
						...
						else if (cond == 2)
						{
							...
							else // Check Hunter license 1 become Hunter license 2
					...
					case CYBELLIN:
						...
						else // Doesn't have Cybellin request
