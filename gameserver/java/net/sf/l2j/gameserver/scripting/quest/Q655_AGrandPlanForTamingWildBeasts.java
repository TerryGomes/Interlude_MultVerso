...
public class Q655_AGrandPlanForTamingWildBeasts extends Quest
{
	...
	private static final String PATH_TO_HTML = "script/siegablehall/WildBeastReserve/messenger_initial.htm";
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("35627-11.htm"))
		{
			...
			{
				htmltext = HtmCache.getInstance().getHtm(player.isLang() + PATH_TO_HTML);
			}
			else
			{
		...
		switch (st.getState())
		{
			case CREATED:
				...
				if (minutesToSiege > 0 && minutesToSiege < MINUTES_TO_SIEGE)
				{
					...
					else if (clan.getClanHallId() == ClanHallSiege.BEAST_FARM && minutesToSiege > 0 && minutesToSiege < MINUTES_TO_SIEGE)
					{
						htmltext = HtmCache.getInstance().getHtm(player.isLang() + PATH_TO_HTML);
					}
					else
				...
				{
					htmltext = getHtmlText(player, "35627-02.htm").replace("%next_siege%", getSiegeDate());
				}
				break;
