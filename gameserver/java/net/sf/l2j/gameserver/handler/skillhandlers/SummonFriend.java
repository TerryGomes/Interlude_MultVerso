...
public class SummonFriend implements ISkillHandler
{
	...
	public void useSkill(Creature activeChar, L2Skill skill, WorldObject[] targets, ItemInstance itemInstance)
	{
		...
		if (skill.getSkillType() == SkillType.SUMMON_PARTY)
		{
			...
			{
				// Check target status.
				if ((member == player) || !checkSummoned(player, member))
				{
					continue;
				}
	...
	public static boolean checkSummoned(Player player, WorldObject target)
	{
		...
		if (!CTFEvent.onEscapeUse(player.getObjectId()) || !DMEvent.onEscapeUse(player.getObjectId()) || !LMEvent.onEscapeUse(player.getObjectId()) || !TvTEvent.onEscapeUse(player.getObjectId()))
		{
			player.sendPacket(SystemMessageId.YOUR_TARGET_IS_IN_AN_AREA_WHICH_BLOCKS_SUMMONING);
			return false;
		}
