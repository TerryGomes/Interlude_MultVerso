...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public void useAdminCommand(String command, Player player)
	{
		...
		else if (command.startsWith("admin_party_info"))
		{
			...
			html.setFile(player.isLang() + "admin/partyinfo.htm");
				...
				case "name":
					try
					{
						...
						if (targetWorldObject instanceof Player)
						{
							...
							// Name is a npc name.
							// Name already exists.
							if (!StringUtil.isValidString(newName, "^[A-Za-z0-9]{1,16}$") || (NpcData.getInstance().getTemplateByName(newName) != null) || (PlayerInfoTable.getInstance().getPlayerObjectId(newName) > 0))
							{
	...
	public static void gatherPlayerInfo(Player player, Player targetPlayer, NpcHtmlMessage html)
	{
		...
		html.setFile(player.isLang() + "admin/charinfo.htm");
		html.replace("%name%", targetPlayer.getName());
