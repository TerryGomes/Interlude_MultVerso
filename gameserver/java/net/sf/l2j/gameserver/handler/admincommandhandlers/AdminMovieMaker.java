...
public class AdminMovieMaker implements IAdminCommandHandler
{
			...
			switch (param)
			{
				case "add":
					if (!st.hasMoreTokens())
					{
						...
						html.setFile(player.isLang() + "admin/movie/add_sequence.htm");
					...
					try
					{
						...
						if (!st.hasMoreTokens())
						{
							...
							final NpcHtmlMessage html = new NpcHtmlMessage(0);
							html.setFile(player.isLang() + "admin/movie/edit_sequence.htm");
	...
	private static void mainHtm(Player player)
	{
		...
		{
			html.setFile(player.isLang() + "admin/movie/main_empty.htm");
		}
		else
		{
			...
			html.setFile(player.isLang() + "admin/movie/main_notempty.htm");
