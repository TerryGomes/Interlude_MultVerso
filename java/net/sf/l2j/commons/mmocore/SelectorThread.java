...
public final class SelectorThread<T extends MMOClient<?>> extends Thread
{
	...
	public SelectorThread(final IMMOExecutor<T> executor, final IPacketHandler<T> packetHandler, final IClientFactory<T> clientFactory, final IAcceptFilter acceptFilter) throws IOException
	{
		super.setName("SelectorThread-" + super.getId());

		DIRECT_WRITE_BUFFER = ByteBuffer.allocateDirect(BUFFER_SIZE).order(BYTE_ORDER);
		WRITE_BUFFER = ByteBuffer.wrap(new byte[BUFFER_SIZE]).order(BYTE_ORDER);
		READ_BUFFER = ByteBuffer.wrap(new byte[BUFFER_SIZE]).order(BYTE_ORDER);
		...
		for (int i = 0; i < Config.HELPER_BUFFER_COUNT; i++)
		{
			_bufferPool.addLast(ByteBuffer.wrap(new byte[BUFFER_SIZE]).order(BYTE_ORDER));
		}
	...
	final ByteBuffer getPooledBuffer()
	{
		...
		{
			return ByteBuffer.wrap(new byte[BUFFER_SIZE]).order(BYTE_ORDER);
		}
	...
	{
		if (_bufferPool.size() < Config.HELPER_BUFFER_COUNT)
		{
			buf.clear();
	...
	private final void readPacket(final SelectionKey key, final MMOConnection<T> con)
	{
		if (!con.isClosed())
		{
			...
			if (result > 0)
			{
				...
				for (int i = 0; i < Config.MAX_READ_PER_PASS; i++)
				{
	...
	private final boolean prepareWriteBuffer(final MMOConnection<T> con)
	{
		...
		if (DIRECT_WRITE_BUFFER.remaining() > 1 && !con.hasPendingWriteBuffer())
		{
			...
			for (int i = 0; i < Config.MAX_SEND_PER_PASS; i++)
			{
				synchronized (con.getSendQueue())
				{
