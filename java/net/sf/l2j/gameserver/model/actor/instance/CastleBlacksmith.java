...
public class CastleBlacksmith extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (!Config.ALLOW_MANOR)
		{
			...
			html.setFile(player.isLang() + "npcdefault.htm");
	...
	public void showChatWindow(Player player, int val)
	{
		...
		{
			html.setFile(player.isLang() + "castleblacksmith/castleblacksmith-no.htm");
		}
		else if (condition == NpcTalkCond.UNDER_SIEGE)
		{
			html.setFile(player.isLang() + "castleblacksmith/castleblacksmith-busy.htm");
		}
		else if (val == 0)
		{
			html.setFile(player.isLang() + "castleblacksmith/castleblacksmith.htm");
		}
		else
		{
			html.setFile(player.isLang() + "castleblacksmith/castleblacksmith-" + val + ".htm");
		}
		html.replace("%objectId%", getObjectId());
		html.replace("%npcname%", getName());
