...
public class PlayerCast extends PlayableCast<Player>
{
	...
	public boolean canAttemptCast(Creature target, L2Skill skill)
	{
		...
		if ((_actor.isSitting() && !_actor.isFakeDeath()) || (_actor.isFakeDeath() && skill.getId() != 60))
		{
	...
	public boolean canDoCast(Creature target, L2Skill skill, boolean isCtrlPressed, int itemObjectId)
	{
		...
		switch (skill.getSkillType())
		{
			...
			case SWEEP:
				...
				{
					if (!_actor.getInventory().validateCapacity(1))
					{
						_actor.sendPacket(SystemMessageId.INVENTORY_LIMIT_MUST_NOT_BE_EXCEEDED);
						return false;
					}
