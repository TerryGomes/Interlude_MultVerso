...
public class Q663_SeductiveWhispers extends Quest
{
			...
			if (param1 == i2)
			{
				...
				if (i3 % 5 == 0 && i3 != 10)
				{
					if ((state % 100) / 10 >= 7)
					{
						...
						rewardItems(player, ENCHANT_ARMOR_A, 1);
						htmltext = getHTML(player, "30846-14.htm", i0, param3, player.getName());
					}
					else
					{
						st.set("state", (state / 10) * 10 + 7);
						htmltext = getHTML(player, "30846-13.htm", i0, param3, player.getName()).replace("%wincount%", String.valueOf(i5 + 1));
					}
				}
				else
				{
					...
					htmltext = getHTML(player, "30846-12.htm", i0, param3, player.getName());
				}
			}
		...
		else if (event.equalsIgnoreCase("30846-19.htm") && (state % 10) == 6 && (state / 1000) == 0)
		{
			...
			if (param1 == i2)
			{
				...
				if (i3 % 5 == 0 && i3 != 10)
				{
					st.set("state", 1);
					st.set("stateEx", 0);
					htmltext = getHTML(player, "30846-19.htm", i0, param3, player.getName());
				}
				else
				{
					st.set("state", (state / 10) * 10 + 5);
					st.set("stateEx", param3);
					htmltext = getHTML(player, "30846-18.htm", i0, param3, player.getName());
				}
			}
			else if (param2 == 5 || i1 == 5)
			{
		...
		else if (event.equalsIgnoreCase("30846-25.htm") && state == 1005)
		{
			...
			if (param1 == i2)
			{
				...
				if (i3 % 5 == 0 && i3 != 10)
				{
					...
					htmltext = getHTML(player, "30846-25.htm", i0, param3, player.getName()).replace("%card1%", String.valueOf(i1));
				}
				else
				{
					st.set("state", 1006);
					st.set("stateEx", param3);
					htmltext = getHTML(player, "30846-24.htm", i0, param3, player.getName());
				}
			}
		...
		else if (event.equalsIgnoreCase("30846-29.htm") && state == 1006)
		{
			...
			if (param1 == i2)
			{
				...
				if (i3 % 5 == 0 && i3 != 10)
				{
					...
					st.set("stateEx", 0);
					rewardItems(player, ADENA, 800);
					htmltext = getHTML(player, "30846-29.htm", i0, param3, player.getName()).replace("%card1%", String.valueOf(i1));
				}
				else
				{
					st.set("state", 1005);
					st.set("stateEx", param3);
					htmltext = getHTML(player, "30846-28.htm", i0, param3, player.getName());
				}
			}
			else if (param2 == 5 || i1 == 5)
			{
				st.set("state", 1);
				st.set("stateEx", 0);
				htmltext = getHTML(player, "30846-29.htm", i0, param3, player.getName());
			}
			else
			{
				st.set("state", 1005);
		...
		switch (st.getState())
		{
			...
			case STARTED:
				...
				else if ((state % 10) == 7)
				{
					...
					{
						htmltext = getHtmlText(player, "30846-16.htm").replace("%wincount%", String.valueOf((state / 10) + 1));
					}
				}
				else if (state == 1005)
				{
	...
	private String getHTML(Player player, String html, int index, int param3, String name)
	{
		return getHtmlText(player, html).replace("%card1pic%", CARDS.get(index).getMessage()).replace("%card2pic%", CARDS.get(param3).getMessage()).replace("%name%", name);
	}
}