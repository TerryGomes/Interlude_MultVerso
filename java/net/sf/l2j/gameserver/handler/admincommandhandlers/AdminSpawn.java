...
public class AdminSpawn implements IAdminCommandHandler
{
	...
	public void useAdminCommand(String command, Player player)
	{
		if (command.startsWith("admin_list_spawns"))
		{
			...
			html.setFile(player.isLang() + "admin/listspawns.htm");
		...
		else if (command.startsWith("admin_spawn"))
		{
			...
			try
			{
				...
				try
				{
					...
					spawn.setRespawnState(!cmd.equalsIgnoreCase("admin_spawn_once"));
					spawn.doSpawn(false);
					SpawnManager.getInstance().addSpawn(spawn, true);

					player.sendMessage("You spawned " + template.getName() + ".");
				}
				catch (Exception e)
				{
